...
def test_flow_efficiency_in_unit_interval() -> None:
    metrics = compute_metrics(simulate(_config()), takt_time=6.0, delivery_window=DELIVERY_WINDOW)
...
def test_littles_law_identity() -> None:
    config = _config()
    log = simulate(config)
    metrics = compute_metrics(log, takt_time=6.0, delivery_window=DELIVERY_WINDOW)
...
def test_smaller_batch_lowers_median_lead_time() -> None:
    ...
    large_batch = compute_metrics(
        simulate(_config(batch_size=10)), takt_time=6.0, delivery_window=DELIVERY_WINDOW
    )
    one_piece = compute_metrics(
        simulate(_config(batch_size=1)), takt_time=6.0, delivery_window=DELIVERY_WINDOW
    )
...
def test_overproduction_inflates_wip_without_higher_throughput() -> None:
    ...
    flood = compute_metrics(
        simulate(_config(release_interval=0.0)), takt_time=6.0, delivery_window=DELIVERY_WINDOW
    )
    # Levelled: release in step with the bottleneck cadence (~weld cycle time).
    levelled = compute_metrics(
        simulate(_config(release_interval=6.0)), takt_time=6.0, delivery_window=DELIVERY_WINDOW
    )
