...
def test_cost_is_priced_on_applied_values() -> None:
    ...
    assert scenario.credit_cost(1000, 0.0) == pytest.approx(15.0)
    # 3.6 rounds to a whole batch of 4 → 1 step.
    assert scenario.credit_cost(3.6, 0.0) == pytest.approx(1.0)
...
def test_api_rejects_over_budget_with_422() -> None:
    response = client.post("/api/simulate", json={"batch_size": 1, "release_interval": 12.0})
